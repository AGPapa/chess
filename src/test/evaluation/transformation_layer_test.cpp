...
TEST(TransformationLayerTest, propagate) {
    std::map<int, std::unique_ptr<TransformationLayer<9>::Weights>> _weights;
    for (int king = 0; king < 64; king++) {
        ...
        _weights[king] = std::move(weight_ptr);
    }

    TransformationLayer<9> layer = TransformationLayer<9>(&_weights, false);
    TransformationLayer<9> flipped_layer = TransformationLayer<9>(&_weights, true);
